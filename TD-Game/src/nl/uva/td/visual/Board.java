...
public class Board extends Group {
    ...
    private final IntegerProperty gameLivesProperty = new SimpleIntegerProperty(0);
    private final IntegerProperty gameMoveLives = new SimpleIntegerProperty(0);
    ...
    private final Label lblLives = new Label();
    ...
    public Board(final GridOperator grid, final GameField gameField) {
        ...
        createScore();
        createGrid(gameField);

        initGameProperties();
    }

    private void createScore() {
        Label lblTitle = new Label("Tower Defence");
        lblTitle.getStyleClass().addAll("game-label", "game-title");
        Label lblSubtitle = new Label("AI");
        ...
        lblLives.getStyleClass().addAll("game-label", "game-lives");
        lblLives.setAlignment(Pos.CENTER);
        lblLives.setMinWidth(50);
        getChildren().add(lblLives);
    ...
    private Rectangle createCell(final int i, final int j, final Field.Type type) {
        ...
        if (type == Field.Type.TOWER_FIELD) {
            cell.getStyleClass().add("game-grid-tower-cell");
        } else {
    ...
    private void createGrid(final GameField gameField) {

        gridOperator.traverseGrid((i, j) -> {
            gridGroup.getChildren().add(createCell(i, j, gameField.getGameField()[i][j].getType()));
            return 0;
        });
    ...
    private void initGameProperties() {
        ...
        // gameLivesProperty.set(i1.intValue());
        // }
        // });
    ...
    public void animateScore() {
        ...
        if (gameMoveLives.get() != 0) {
            ...
            final Timeline timeline = new Timeline();
            lblLives.setText("-" + gameMoveLives.getValue().toString());
            lblLives.setOpacity(1);
            double posX = vScore.localToScene(vScore.getWidth() / 2d, 0).getX();
            lblLives.setTranslateX(0);
            lblLives.setTranslateX(lblLives.sceneToLocal(posX, 0).getX() + lblLives.getWidth());
            lblLives.setLayoutY(20);
            final KeyValue kvO = new KeyValue(lblLives.opacityProperty(), 0);
            final KeyValue kvY = new KeyValue(lblLives.layoutYProperty(), 100);
    ...
    public Tile addRandomTile(final Location randomLocation) {
    ...
    public void removeLives(final int lives) {
        gameMoveLives.set(gameMoveLives.get() + lives);
        gameLivesProperty.set(gameLivesProperty.get() - lives);
    }
    ...
    public void setGameWin(final boolean won) {
    ...
    public void saveSession(final Map<Location, Tile> gameGrid) {
        sessionManager.saveSession(gameGrid, gameScoreProperty.getValue(),
                LocalTime.now().minusNanos(time.toNanoOfDay()).toNanoOfDay());
    }

    public boolean restoreSession(final Map<Location, Tile> gameGrid) {
        doClearGame();
        timer.stop();
    ...
    private void restoreRecord() {
        // RecordManager recordManager = new RecordManager(gridOperator.getGridSize());
        // gameLivesProperty.set(recordManager.restoreRecord());
    }
